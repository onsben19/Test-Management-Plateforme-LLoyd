...
SCENARIOS = [
    (
        ...
        "TC-AUTH-001",
        {
            "action": "Aller sur http://localhost/login. Saisir l'email tester@lloyd.com et le mot de passe Test1234!. Cliquer sur Se connecter. Vérifier que la page demande un code OTP. Saisir le code 000000. Vérifier que le tableau de bord s'affiche.",
    ...
    (
        ...
        "TC-SOUSCR-001",
        {
            "action": "Se connecter sur http://localhost/login avec manager@lloyd.com / Test1234!. Aller sur /manager. Vérifier que le kanban des campagnes s'affiche avec au moins une colonne.",
    ...
    (
        ...
        "TC-REGR-001",
        {
            "action": "Se connecter sur http://localhost/login. Aller sur /anomalies. Vérifier que la liste des anomalies s'affiche. Vérifier que le bouton Nouvelle anomalie est visible.",
            "expected": "La page anomalies charge correctement avec la liste et le bouton de création.",
            "url": "http://localhost/anomalies"
        }
    ),
    (
        "Sécurité",
        "TC-SEC-001",
        {
            "action": "Ouvrir http://localhost/login. Saisir des identifiants incorrects (email: faux@test.com, mot de passe: mauvais). Cliquer sur Se connecter. Vérifier qu'un message d'erreur apparaît et que l'utilisateur reste sur la page de connexion.",
    ...
    (
        ...
        "TC-MOB-001",
        {
            "action": "Ouvrir http://localhost sur un viewport de 375x812. Vérifier que la page s'affiche sans débordement horizontal. Vérifier que le logo InsureTM est visible.",
    ...
    (
        ...
        "TC-EXP-001",
        {
            "action": "Se connecter sur http://localhost/login. Aller sur /anomalies. Vérifier que la page des anomalies se charge. Vérifier qu'il y a au moins un élément dans la liste.",
    ...
    (
        ...
        "TC-API-001",
        {
            "action": "Se connecter sur http://localhost/login. Aller sur /analytics. Vérifier que la page Analytics se charge. Vérifier que le widget de chat IA est visible ou qu'un bouton pour l'ouvrir est présent.",
    ...
    (
        ...
        "TC-RECT-001",
        {
            "action": "Se connecter sur http://localhost/login. Aller sur /portfolio. Vérifier que la liste des projets business s'affiche. Vérifier qu'au moins un projet est visible.",
    ...
    (
        ...
        "TC-NOTIF-001",
        {
            "action": "Se connecter sur http://localhost/login. Vérifier que l'en-tête contient une icône de notification (cloche). Cliquer sur l'icône. Vérifier que le panneau de notifications s'ouvre.",
    ...
    (
        ...
        "TC-TARIF-001",
        {
            "action": "Se connecter sur http://localhost/login. Aller sur /manager/dashboard. Vérifier que le tableau de bord manager se charge. Vérifier qu'au moins un indicateur KPI est affiché.",
            "expected": "Le tableau de bord manager affiche les KPIs et le Readiness Score.",
            "url": "http://localhost/manager/dashboard"
        }
    ),
    (
        "Compatibilité",
        "TC-COMPAT-001",
        {
            "action": "Ouvrir http://localhost/login. Vérifier que le formulaire de connexion est affiché avec les champs email et mot de passe. Vérifier que le titre InsureTM est visible sur la page.",
    ...
    (
        ...
        "TC-RGPD-001",
        {
            "action": "Se connecter sur http://localhost/login. Aller sur /profile. Vérifier que la page de profil se charge. Vérifier que les champs nom, prénom et email sont visibles.",
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        self.stdout.write(
            ...
            "   http://localhost/tester-dashboard\n"
