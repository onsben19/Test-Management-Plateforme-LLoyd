            ...
            <div className="max-w-2xl space-y-8 pb-10">
                ...
                >
                    <div className="flex items-center gap-4 mb-5">
                        ...
                        <div>
                            <h3 className="text-[15px] font-semibold text-white">Informations personnelles</h3>
                    ...
                    <form onSubmit={handleSave}>
                        ...
                        <div className="flex items-center gap-5 mb-8">
                            ...
                            <div className="relative shrink-0">
                                ...
                                <button type="button" onClick={() => fileInputRef.current?.click()}
                                    className="absolute bottom-0 right-0 p-1.5 bg-[#1a2235] rounded-full border border-white/10 text-slate-400 hover:text-white transition-colors shadow-lg"
                            ...
                            <div className="flex flex-col gap-1.5">
                                <span className="text-[14px] font-semibold text-white">{fullName}</span>
                                <span className="text-[12px] text-slate-400">{user?.email}</span>
                                <button type="button" onClick={() => fileInputRef.current?.click()}
                                    className="text-[11px] text-slate-300 hover:text-white px-3 py-1.5 rounded-lg border border-white/10 hover:bg-white/5 transition-colors w-fit mt-0.5"
                                >
                                    Changer la photo
                                </button>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-bold text-white/30 uppercase tracking-widest ml-1">Prénom</label>
                                <div className="flex items-center gap-2.5 px-3.5 py-[11px] bg-white/[0.02] border border-white/[0.05] rounded-[10px]">
                                    <span className="text-[13px] text-white/40">{user?.first_name || '—'}</span>
                                </div>
                            </div>
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-bold text-white/30 uppercase tracking-widest ml-1">Nom</label>
                                <div className="flex items-center gap-2.5 px-3.5 py-[11px] bg-white/[0.02] border border-white/[0.05] rounded-[10px]">
                                    <span className="text-[13px] text-white/40">{user?.last_name || '—'}</span>
                                </div>
                            </div>
                        ...
                        <div className="space-y-1.5 mb-6">
                            <label className="text-[10px] font-bold text-white/50 uppercase tracking-widest ml-1">Téléphone</label>
                        ...
                        <div className="flex justify-end gap-3">
                            <button type="button" onClick={handleCancel}
                                className="px-5 py-2 rounded-[10px] text-[13px] font-medium text-slate-400 hover:text-white hover:bg-white/5 transition-colors border border-transparent"
                            >
                                Annuler
                            </button>
                            <button type="submit" disabled={saving}
                                className="flex items-center gap-2 px-5 py-2 rounded-[10px] text-[13px] font-semibold text-white bg-blue-600 hover:bg-blue-500 transition-colors shadow-lg shadow-blue-500/20 disabled:opacity-60"
                            >
                ...
                >
                    <div className="flex items-center gap-4 mb-5">
                        ...
                        <div>
                            <h3 className="text-[15px] font-semibold text-white">Sécurité</h3>
                            <p className="text-slate-400 text-[12px] mt-0.5">Réinitialisation du mot de passe par email</p>
                        </div>
                    </div>
                    <div className="h-px bg-white/[0.06] mb-6" />

                    <div className="flex items-start gap-4 p-4 bg-white/[0.02] border border-white/[0.06] rounded-xl mb-6">
                        <ShieldCheck size={18} className="text-blue-400 shrink-0 mt-0.5" />
                        <div>
                            <p className="text-[13px] font-medium text-white/80">Réinitialiser le mot de passe</p>
