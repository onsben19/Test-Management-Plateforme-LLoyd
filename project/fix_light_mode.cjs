...
function processFile(filePath) {
    ...
    content = content.replace(/(?<!dark:)bg-\[#0B1120\]/g, 'bg-slate-50 dark:bg-[#0B1120]');
    ...
    content = content.replace(/className=(['"\`])(.*?)\1/g, (match, quote, classes) => {
        // We only split if it's a simple string, but since we have conditional templates like `foo ${bar}` this can be complex.
        // We will just do a simple replacement if the WHOLE string does not contain bg-blue, etc.
        const hasSolidBg = 
            ...
            classes.includes('bg-[#3b82f6]') || 
            classes.includes('bg-[#fb923c]') ||
            classes.includes('from-') || 
            classes.includes('via-');

        if (!hasSolidBg) {
            // Safe to replace text-white with text-slate-900 dark:text-white
            // Be careful to use word boundaries to not replace hover:text-white as text-slate-900...
            let newClasses = classes;
            newClasses = newClasses.replace(/(?<!dark:)(?<!hover:)(?<!focus:)(?<!group-hover:)\btext-white\b/g, 'text-slate-900 dark:text-white');
            newClasses = newClasses.replace(/(?<!dark:)\bhover:text-white\b/g, 'hover:text-slate-900 dark:hover:text-white');
            newClasses = newClasses.replace(/(?<!dark:)\bgroup-hover:text-white\b/g, 'group-hover:text-slate-900 dark:group-hover:text-white');
