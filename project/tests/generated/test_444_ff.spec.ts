...
test.use({ screenshot: 'on', baseURL: 'http://nginx', storageState: 'tests/tester/.auth/tester.json', actionTimeout: 15000, navigationTimeout: 15000 });
...
test('ff', async ({ page }) => {
  ...
  const usernameInput = page.locator('input[name="username"], input[type="text"], input[id*="username"], textarea[id*="username"], select[id*="username"]');
  const passwordInput = page.locator('input[name="password"], input[type="password"], input[id*="password"], textarea[id*="password"], select[id*="password"]');

  await usernameInput.first().evaluate((el, val) => { el.value = val; el.dispatchEvent(new Event('input', {bubbles: true})); el.dispatchEvent(new Event('change', {bubbles: true})); }, 'tam');
