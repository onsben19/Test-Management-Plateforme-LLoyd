...
test('ref', async ({ page }) => {
  ...
  const welcomeMessage = page.locator('div.flash, div.success, [role="alert"]');
  await expect(welcomeMessage.first()).toContainText('Welcome to the Secure Area. When you are done browsing, sign out.');
});